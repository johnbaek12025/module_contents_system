...
class Contents(ADManager):
    ...
    def make_news_info(self):
        temp = f"{self.accumulated_days}일 {get_size_str(self.dealing_type)} {get_size_symbol(self.dealing_type)}"
        news_title_list = [
            {
    ...
    def get_bottom_tr(self):
        ...
        return f"""
            <div class="module-tbl-txt mt-20">
                <div class="dot"> {self.accumulated_days}일 누적 {r1["sub"]} {r1["dt"]}: {r1["abs_val"]} {r1["unit"]} (<span class={r1["size_style"]}>상장주식수대비 {r1["ratio"]}%</span>)</div>
                <div class="dot"> {self.accumulated_days}일 누적 {r2["sub"]} {r1["dt"]}: {r1["abs_val"]} {r2["unit"]}</div>
